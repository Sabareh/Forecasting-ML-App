...
server <- function(input, output, session) {
  ...
  output$top5plot <- renderPlotly({
    ...
    # Get the first product's data for initial plot
    first_product <- names(dc)[2]  # First product column
    first_product_data <- dc[[first_product]]
    
    # Create hover text for first trace
    hovertext0 <- paste0(dc$Month, ":<b> ", first_product, " ", first_product_data)
    
    # Initialize plot with first product
    pplot <- plot_ly(data = dc, 
                     x = ~Month, 
                     y = as.formula(paste0("~`", first_product, "`")),
                     type = "scatter", 
                     mode = "lines", 
                     name = first_product, 
                     line = list(color = colorp[1], width = 2.5), 
                     hoverinfo = "text", 
                     text = hovertext0)
    ...
    for (i in 3:ncol(dc)) {
      product_name <- names(dc)[i]
      product_data <- dc[[product_name]]
      hovertext <- paste0(dc$Month, ":<b> ", product_name, " ", product_data)
      
      pplot <- pplot %>% 
        add_trace(y = as.formula(paste0("~`", product_name, "`")),
                  name = product_name, 
                  line = list(color = colorp[i-1], width = 2.5), 
                  hoverinfo = "text", 
                  text = hovertext)
    }
